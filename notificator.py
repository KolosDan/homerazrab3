...
while True:
    notifications = [(i['from'], i['to']) for i in db.notifications.find()]
    try:
        if set(old_notifications) - set(notifications) != 0:
            for from_, to_ in set(old_notifications) - set(notifications):
